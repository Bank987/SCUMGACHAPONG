...
export default function Admin() {
  ...
  const handleDeleteUser = async (targetUser: any) => {
    ...
    if (!confirm(`ล้างข้อมูลของ ${label} ทั้งหมดหรือไม่? ผู้เล่นต้อง LOGIN ใหม่และไอเทม/ประวัติจะถูกลบ`)) return;
    const res = await adminFetch(`/api/admin/users/${targetUser._id}`, { method: "DELETE" });
    if (res.ok) {
      toast.success(`ล้างข้อมูล ${label} แล้ว`);
      loadData();
    } else {
      toast.error("ล้างข้อมูลผู้ใช้ไม่สำเร็จ");
    }
  };

  const handleWipeUsers = async () => {
    const confirmation = prompt('การทำงานนี้จะลบบัญชี ไอเทม ประวัติ และ log ของผู้ใช้ทั้งหมด\nพิมพ์คำว่า WIPE เพื่อยืนยัน');
    if (confirmation !== "WIPE") return;
    const res = await adminFetch("/api/admin/users", { method: "DELETE" });
    if (res.ok) {
      setUsers([]);
      setLogs([]);
      toast.success("WIPE ข้อมูลผู้ใช้ทั้งหมดเรียบร้อย");
    } else {
      toast.error("WIPE ไม่สำเร็จ");
    }
  };
        ...
        {activeTab === "users" && (
          <div>
            <div className="mb-8 flex flex-wrap items-center justify-between gap-4">
              ...
              <button onClick={handleWipeUsers} className="flex items-center gap-2 rounded-xl border border-red-500/40 bg-red-500/10 px-4 py-2.5 text-sm font-black text-red-400 transition hover:bg-red-600 hover:text-white">
