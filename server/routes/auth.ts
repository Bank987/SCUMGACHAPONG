...
router.put("/game-name", async (req, res) => {
  try {
    const userId = getUserId(req);
    if (!userId) return res.status(401).json({ error: "Unauthorized" });

    const gameName = typeof req.body?.gameName === "string" ? req.body.gameName.trim() : "";
    if (gameName.length < 2 || gameName.length > 32) {
    ...
    const user = await User.findByIdAndUpdate(userId, { $set: { gameName } }, { new: true });
    if (!user) return res.status(404).json({ error: "User not found" });
    res.json(user);
  } catch (err) {
    console.error("Set game name error:", err);
    res.status(500).json({ error: "Server error" });
  }
});
